...
import com.trantanphong.convert.ResearchStudentStatisticalConvert;
...
//	private static Logger log = LogManager.getLogger(ResearchServiceImp.class);
	...
	@Autowired
	private ResearchStudentStatisticalConvert researchStudentStatisticalConvert;
	...
	public void updateFileName() {
		...
		for (Research research : researchs) {
			if (research.getReportDocumnetCode() != null) {
				research.setReportFileName(alfrescoService.fileName(research.getReportDocumnetCode()));
				research = researchRepository.save(research);
			}
	...
	@Override
	public List<ResearchDTO> getAllResearchBySchoolYearStatus(long schoolId, long statusId) {
		// TODO Auto-generated method stub
		List<ResearchDTO> dtos = new ArrayList<>();
		List<Research> researchs = researchRepository.getAllResearchSchoolYearStatus(schoolId, statusId);
		for (Research research : researchs) {
			ResearchDTO dto = researchConvert.toDTO(research);
			DetailStatusDTO detailStatus = detailStatusService.getStatusNow(dto.getResearchID());
			// if (detailStatus.getStatusID() != 8) {
	...
	@Override
	public List<ResearchDTO> getAllResearchByStatus(long statusId) {
		// TODO Auto-generated method stub
		List<ResearchDTO> dtos = new ArrayList<>();
		List<Research> researchs = researchRepository.getAllResearchStatusFaculty(statusId);
		for (Research research : researchs) {
			ResearchDTO dto = researchConvert.toDTO(research);
			DetailStatusDTO detailStatus = detailStatusService.getStatusNow(dto.getResearchID());
			// if (detailStatus.getStatusID() != 8) {
	...
	@Override
	public List<ResearchStudentStatistical> getResearchStudentStatisticalBySchoolYear(long schoolYearId) {
		// TODO Auto-generated method stub
		SchoolYearDTO dto = schoolYearService.getSchoolYearByID(schoolYearId);
		List<ResearchStudentStatistical> researchStudentStatisticals = new ArrayList<>();
		List<Tuple> tuples = researchRepository.getStatisticalResearchStudent(schoolYearId);
		for (Tuple tuple : tuples) {
			ResearchStudentStatistical researchStudentStatistical = researchStudentStatisticalConvert.toDTOBySchoolYear(tuple);
			if(dto != null) {
				researchStudentStatistical.setSchoolYearName(dto.getSchoolYearName());
			}
			researchStudentStatisticals.add(researchStudentStatistical);
		}
		return researchStudentStatisticals;
	}

	@Override
	public List<ResearchStudentStatistical> getResearchStudentStatisticalAllSchoolYear() {
		List<ResearchStudentStatistical> researchStudentStatisticals = new ArrayList<>();
		List<Tuple> tuples = researchRepository.getStatisticalResearchStudentAllSchoolYear();
		for (Tuple tuple : tuples) {
			ResearchStudentStatistical researchStudentStatistical = researchStudentStatisticalConvert.toDTO(tuple);
			researchStudentStatisticals.add(researchStudentStatistical);
		}
		return researchStudentStatisticals;
	}
