...
public class StudentServiceImp implements StudentService {
	...
	public long[] getStudentNotResearch() {
		...
		long [] result = new long[subjectDTOs.size()];
		//List<Student> studentsAll = studentRepository.getAllStudentNotResearch();
		//result[0] = studentsAll.size();
		int index = 0;
		for (SubjectDTO subjectDTO : subjectDTOs) {
			List<Student> students = studentRepository.getAllStudentNotResearchBySubject(subjectDTO.getSubjectCode());
			result[index] = students.size();
			index++;
		}
		return result;
	}
	...
	public long getSumStudentNotResearch() {
		// TODO Auto-generated method stub
		List<Student> students = studentRepository.getAllStudentNotResearch();
